...
<div class="container-fluid">
	<form action="" id="product-form">
		...
		<div class="form-group">
			<label for="name" class="control-label">Megnevezés</label>
		...
		<div class="form-group">
			<label for="price" class="control-label">Ár</label>
		...
		<div class="form-group">
			...
			<select name="status" id="status" class="form-control form-control-sm rounded-0" required>
				<option value="1" <?php echo isset($status) && $status == 1 ? 'selected' : '' ?>>Aktív</option>
				<option value="0" <?php echo isset($status) && $status == 0 ? 'selected' : '' ?>>Inaktív</option>
			</select>
		</div>
		<div class="form-group">
			<label for="" class="control-label">Kép</label>
			<div class="custom-file">
				...
				<label class="custom-file-label" for="customFile1">Fájl kiválasztása</label>
			</div>
		</div>
		<div class="form-group">
	...
	$(document).ready(function(){
		$('#product-form').submit(function(e){
			...
			$.ajax({
				...
				error:err=>{
					console.log(err)
					alert_toast("Hiba lépett fel. Kérem figyeljen oda a kép feltöltése során a kép formátuma .jpeg kiterjesztésű legyen!",'error');
					end_loader();
				},
				success:function(resp){
					if(typeof resp =='object' && resp.status == 'success'){
						...
						alert_toast("Hiba lépett fel.",'error');
						end_loader();
					}
				}
			})
